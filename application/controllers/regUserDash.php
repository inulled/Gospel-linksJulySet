...
class regUserDash extends CI_Controller {
	public function postToWall() {
		...
		$this->db->query("INSERT IGNORE INTO friends (node1id, node2id, relationSince, relationType)
		...
		$this->db->query("UPDATE friends SET relationType = 'friends' WHERE node1id = '{$userid}' AND node2id = '{$targetedUserId}'");
		...
		$this->db->query("DELETE FROM friends WHERE node1id = '{$userid}' AND node2id = '{$targetedUserId}'");
